...
export function ArtistInfo({artist,onClose}:{artist:Artist;onClose:()=>void}) {
  ...
  return <Modal transparent animationType="fade" onRequestClose={onClose}><View style={s.overlay}><View style={s.sheet}>
    ...
    <Text style={s.heading}>代表作</Text>{artist.works?.length?artist.works.map((work,i)=><Text key={i} style={s.line}>{work.name} · {[work.language,work.category,work.released].filter(Boolean).join(' / ')}</Text>):<Text style={s.line}>暂无已核实资料</Text>}
...
export function EventArtistBackground({id}:{id:string}) {
  const [data,setData]=useState<{artists:Artist[];references:Reference[]}|null>(null);
  ...
  if(!data||(!data.artists.length&&!data.references.length))return null;
  return <View style={{gap:14,marginVertical:20}}><Text style={s.heading}>阵容与背景资料</Text><ArtistNames artists={data.artists}/><References items={data.references}/></View>;
}
