...
try {
  ...
  assert.equal(await loginButton.isEnabled(), true, 'login action stays clickable so it can report validation errors');
  assert.equal(await loginButton.evaluate((element) => getComputedStyle(element).cursor), 'pointer');
  await loginButton.click();
  await page.getByRole('alert').getByText('请输入邮箱', { exact: true }).waitFor();
  await page.getByRole('button', { name: '没有账号？注册' }).click();
  const registerButton = page.getByRole('button', { name: '注册并登录', exact: true });
  assert.equal(await registerButton.isEnabled(), true, 'registration action stays clickable so it can report validation errors');
  assert.equal(await registerButton.evaluate((element) => getComputedStyle(element).cursor), 'pointer');
