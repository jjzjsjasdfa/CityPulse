...
try{
 ...
 await page.getByText('账号资料已保存',{exact:true}).waitFor();await page.getByText('微信 · 暂未开放',{exact:true}).waitFor();
 await page.getByRole('button',{name:'返回我的'}).click();await page.getByText('城市漫游者',{exact:true}).waitFor();
 ...
 await page.getByRole('button',{name:'设置',exact:true}).click();await page.getByLabel('开启调试功能').click();await page.getByRole('button',{name:'应用设置并返回地图'}).click();
 ...
 await page.getByRole('button',{name:'我的',exact:true}).click();await page.getByRole('button',{name:'登录 / 注册'}).waitFor();
 if(await page.getByRole('button',{name:'审核',exact:true}).count())throw Error('Admin tab survives logout');
 if(errors.length)throw Error(errors.join('\n'));
 console.log(JSON.stringify({passed:true,checks:['guest bypass and map','regular profile saved','WeChat placeholder','debug role isolation','debug approval notice','logout while debugging'],errors}));
}finally{
 await browser?.close();
 db(`import json,sys
